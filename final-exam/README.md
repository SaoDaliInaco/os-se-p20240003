# Final Exam — Sao Dali Inaco

<!-- ===== COVER SHEET — required first section. Fill EVERY line. ===== -->
```
Student name: Sao Dali Inaco
Student ID: p20240003
Server username: se-sao-dali-inaco
Exam scenario value (COMPANY / PRODUCT): Techcorp/Beacon
Date & start time: 30/06/2026
...
**Written (one short answer)**
> Why setuid_demo doesn't escalate to root: Because setuid_demo is owned by your own student account (UID 1140), not root. The setuid bit makes the process run with the file owner's UID as its effective UID — since the owner is just you, the effective UID stays 1140 (your own), never reaching root's UID 0. There's no sudo or root ownership involved anywhere in the chain.
...
**Written (one short answer)**
>The shell only searches the directories listed in the PATH environment variable when resolving a bare command name. Since ~/bin wasn't part of PATH at that point, the shell had no way to locate greeter — it would return "command not found." Running it as ./greeter worked because that explicitly specifies the file's location rather than relying on PATH lookup.
